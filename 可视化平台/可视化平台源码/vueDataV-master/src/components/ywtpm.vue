...
<template>
  <div class="wrap-container sn-container">
    <div class="sn-content">
      <div class="sn-title" id="title">
        <div id="titleText">业务庭排名</div>
      </div>
      <div class="sn-body">
            ...
            <div id="chart1" class="chart-1">
              <div class="compass">
                ...
                <div class="compass-text" :class="[`compass-text-${index + 1}`, {show: number == index}]" v-for="(item, index) in arr" :key="index">
                  <span>当月结案数：{{ item.value1 }}</span>
                  <span>当月未结数：{{ item.value2 }}</span>
                </div>
...
export default {
  ...
  mounted() {
    ...
    let that=this;
    this.axios.get('/getYwtpm').then(function (response){
      if (response.data.code==0){
        that.arr=response.data.chartData;
      } else{
        // 失败
      }
